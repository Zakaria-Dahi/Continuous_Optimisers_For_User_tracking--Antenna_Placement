...
%% Mapping Technique : Sigmoid Function
...
%% variable used during the calcul ----------------------------------------
     ...
     fitness_best = 0;
     %%%% save the continous produced population 
     population = [];
     %%%% save the binary produced population
     ...
     populationsave = [];
     %%% save the previous version of the produced binary population 
     population_bin_save = [];
     %%% save the randomly generated matrix to have the same results (i.e. fitnesses)
     RANDOM_POPULATION_save = [];
     %%% save the best random solution used to generate the best binary one
     gbest_rand = [];
...
%% ---------------- Test in case there is invalid solution ------------------------------
for w=1:indiv
    if population_bin(w,:) ==  zeros(1,Dimension)
	    population_bin(w,randi(Dimension)) =  1;
    end
end
...
fit_new = [];
%% Save the previous randomly generated solutions ------------------
RANDOM_POPULATION_save = RANDOM_POPULATION;
%% ------------ Generate the initial random matrix -----------------
RANDOM_POPULATION = rand(indiv,Dimension);
RANDOM_POPULATION(indbest,:) = gbest_rand;
% ------------------------------------------------------------------
%% ------- memorize the previous state of binary and continuous coded population   ---------
populationsave = population;
population_bin_save = population_bin;
...
Q_save = Q;
% -----------------------------------------------------------------
% ***************** Apply the local and global search of the Bat Algorithm *************************
for  w=1:indiv
...
for  w=1:indiv     
 %% ----- If new solutions are better, update them in   the population ------
           if  (fit_new(w) > fit(w)) || (isequal(num2str(fit_new(w)),'NaN') == 1)
               ...
               population_bin(w,:) = population_bin_save(w,:);
               RANDOM_POPULATION(w,:) = RANDOM_POPULATION_save(w,:);
               v(w,:) = v_save(w,:);
               A(w) = A_save(w);
               r(w) = r_save(w);
               Q(w) = Q_save(w);
